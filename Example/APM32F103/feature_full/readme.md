...
This demo provides an interactive command-line shell for the LittleFS file system on the APM32F103 internal flash. Users can perform file system operations through USART1 using Linux-like commands, creating a persistent storage experience where data remains available across resets.
...
本 Demo 为 APM32F103 内部 Flash 上的 LittleFS 文件系统提供交互式命令行 Shell。用户可以通过 USART1 使用类 Linux 命令执行文件系统操作，创建持久化存储体验，数据在复位后仍然可用。
